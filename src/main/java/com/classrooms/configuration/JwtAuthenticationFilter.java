...
public class JwtAuthenticationFilter extends UsernamePasswordAuthenticationFilter {
	...
	public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response) {
		...
		System.out.println("HHHHHHHHHH33");
		UserApp user = null;
		// User user = new User("youssef","youssef");
		try {
		...
		System.out.println("user : " + user.getUsername() + " : " + user.getPassword() + " : " + user.getRoles());
...
//		System.out.println("UserPrincipal : " + principal.getAuthorities());
	...
	/*	Claims claims = Jwts.claims().setSubject(principal.getUsername());
		...
		String token = Jwts.builder().setClaims(claims).signWith(SignatureAlgorithm.HS512, JwtProperties.SECRET)
				.compact();*/
...
//				.setExpiration(new Date(now + JwtProperties.EXPIRATION_TIME)) 
